...
export default function Cart({onClose}) {
    ...
    return (
        <div onClick={onClose} className='fixed inset-0 bg-slate-700/75 flex justify-center items-center'>
            ...
            className='z-20 bg-slate-300 text-slate-900 min-w-[400px] md:min-w-[700px] px-10 pt-10 pb-6 rounded border border-slate-600  relative'
            >
                <button 
                onClick={onClose}
                ...
                <ul>
                    {cart.cartItems.length > 0 ? cart.cartItems.map(el =>(
                        <li key = {el.id} className='flex items-center mb-4'>
                            <img 
                            className='w-16 h-16 rounded'
                            src={`/images/${el.img}.png`} alt={el.title} />
                ...
                <p className='text-xl'>Your total : {cart.cartItems.reduce((acc, curr) => acc + curr.price * curr.quantity,0).toFixed(2)}$</p>
